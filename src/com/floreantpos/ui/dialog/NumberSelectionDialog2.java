...
public class NumberSelectionDialog2 extends POSDialog implements ActionListener {
	...
	private void init() {
		...
		MigLayout layout = new MigLayout("fillx", "[40px,fill][40px,fill][40px,fill]", "[][][][][]");
		contentPane.setLayout(layout);

		titlePanel = new TitlePanel();
		contentPane.add(titlePanel, "spanx ,growy,height 40,wrap");

		tfNumber = new JTextField();
		tfNumber.setText(String.valueOf(defaultValue));
		tfNumber.setFont(tfNumber.getFont().deriveFont(Font.BOLD, 14));
		...
		posButton.setMinimumSize(new Dimension(13, 11));
		...
		for (int i = 0; i < numbers.length; i++) {
			for (int j = 0; j < numbers[i].length; j++) {
				...
				posButton.addActionListener(this);
				String constraints = "grow, height 45";
